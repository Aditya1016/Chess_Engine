...
int CheckBoard(const S_BOARD *pos)
{
    ...
    while (t_pawns[WHITE])
    {
        ...
        if (pos->pieces[SQ120(sq64)] != wP){
            // printf("Error wP bitboard\n");
            // printf("pos->pieces[%d] = %d\n", SQ120(sq64), pos->pieces[SQ120(sq64)]);
            // PrintBoard(pos);
        }
...
void UpdateListsMaterial(S_BOARD *pos)
{
    ...
    for (index = 0; index < BRD_SQ_NUM; ++index)
    {
        ...
        if (piece != OFFBOARD && piece != EMPTY)
        {
            ...
            // printf("UpdateListsMaterial: piece %d on %d\n", piece, sq);
            if (piece == wP)
            {
